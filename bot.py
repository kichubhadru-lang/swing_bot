...
def relative_strength(stock_df, benchmark_df, lookback=60):
    try:
        ...
        lb = min(lookback, len(stock_df) - 1, len(benchmark_df) - 1)
...
def weekly_trend(symbol):
    try:
        weekly = download_stock(symbol, "3y", "1wk")
        ...
        weekly["EMA20"] = EMAIndicator(close, 20).ema_indicator()
        weekly["EMA50"] = EMAIndicator(close, 50).ema_indicator()
        ...
        return latest["Close"] > latest["EMA20"] > latest["EMA50"]
